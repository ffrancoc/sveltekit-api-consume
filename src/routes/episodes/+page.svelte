<script lang="ts">
	...
	import type { EpisodeType, InfoType } from '../../interfaces/interfaces';

	let info: InfoType;
	info = $page.data['info'];

	let results: Array<EpisodeType>;
	...
	const onPrevLocation = async () => {
		if (nPage > 1 && nPage <= totalPages) {
			nPage = nPage - 1;
			const resp = await fetch(`api/episodes?page=${nPage}`);
...
<div class="container p-4 bg-dark text-white">
	<div class="d-flex gap-2 mb-2">
		...
		<h3 class="flex-grow-1 text-center text-uppercase">Episodios</h3>
	</div>
	<div
